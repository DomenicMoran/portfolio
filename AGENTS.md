...
Sechzehn der Prüfläufe öffnen einen Browser und messen an der ausgelieferten
Seite: `check:a11y`, `check:cards`, `check:focus`, `check:font-size`,
`check:headings`, `check:language`, `check:lighthouse`, `check:links`,
`check:panels`, `check:parity`, `check:print`, `check:privacy`,
`check:separators` und `check:vitals`. Sie sind der eigentliche Beweis.
